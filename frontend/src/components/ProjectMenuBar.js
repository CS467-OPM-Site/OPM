...
import '../styles/ProjectMenuBar.css';
...
const ProjectMenuBar = ({ 
  ...
  const handleAddingColumn = async () => {
    ...
    setIsLoading(true);
    const response = await addColumn(columnTitleToAdd, location.state.projectID);
  ...
  const handleDeletingProject = async() => {
    ...
    const response = await deleteProject(idToken, location.state.projectID);
